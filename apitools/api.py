...
class CanvasAPI():
    ...
    def get(self, api, to_json=True, payload=None, single=False):
        ...
        if single:
            # print responses
            #logging.warning ('responses*******************' +str(responses) + '*********************responses')
            return responses[0]
        else:
            # print responses
            #logging.warning ('responses*******************' +str(responses) + '*********************responses')
    ...
    def getCanvasID(self, OUNetID):
        return self.get('/accounts/%s/users?search_term=%s' % (settings.PACS_SUBACCOUNT, OUNetID), single=True)
    
    
    
    def get_class_by_teacher(self, canvasID):
        #log = "https://canvas.ou.edu/api/v1/accounts/"+ str(settings.PACS_SUBACCOUNT) +"/courses?published=true&by_teachers[]="+str(canvasID)+"&enrollment_term_id=" + str(settings.NEXT_TERM)
        #logging.warning ('log*******************' + log + '*********************log')
        return self.get('/accounts/%s/courses?published=true&by_teachers[]=%s&enrollment_term_id=%s' % (settings.PACS_SUBACCOUNT, canvasID, settings.NEXT_TERM))
